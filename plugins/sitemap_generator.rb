...
module Jekyll
  ...
  MY_URL = "https://xively.com"

  # Change SITEMAP_FILE_NAME if you would like your sitemap file
  # to be called something else
  SITEMAP_FILE_NAME = "sitemap-pages.xml"

  # Any files to exclude from being included in the sitemap.xml
  EXCLUDED_FILES = ['atom.xml', '403.html', '404.html', '500.html', '503.html']

  # Any paths to exclude from being included in the sitemap.xml
  EXCLUDED_PATHS = ['^/static_partials']
  ...
  class SitemapGenerator < Generator
    ...
    def generate_index(site)
      index = REXML::Document.new << REXML::XMLDecl.new("1.0", "UTF-8")

      sitemapindex = REXML::Element.new "sitemapindex"
      sitemapindex.add_attribute("xsi:schemaLocation", "http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/siteindex.xsd")
      sitemapindex.add_attribute("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance")

      index.add_element(sitemapindex)

      sitemapindex.add_element(build_sitemap('sitemap-pages.xml'))
      sitemapindex.add_element(build_sitemap('sitemap-feeds.xml'))

      file = File.new(File.join(site.dest, 'sitemap.xml'), "w")
      formatter = REXML::Formatters::Pretty.new(4)
      formatter.compact = true
      formatter.write(index, file)
      file.close
      site.static_files << Jekyll::SitemapFile.new(site, site.dest, "/", 'sitemap.xml')
    end

    def build_sitemap(filename)
      sitemap = REXML::Element.new "sitemap"
      loc = sitemap.add_element(REXML::Element.new("loc"))
      loc.text = "#{MY_URL}/#{filename}"
      lastmod = sitemap.add_element(REXML::Element.new("lastmod"))
      lastmod.text = Time.now.iso8601
      sitemap
    end
    ...
    # Returns boolean
    def excluded?(page)
      EXCLUDED_FILES.include?(page.name) || EXCLUDED_PATHS.map{|p| !!page.url.match(p)}.include?(true)
    end
